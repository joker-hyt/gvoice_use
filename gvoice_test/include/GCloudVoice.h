...
namespace gcloud_voice
{
    ...
    public:
         ...
         ** Deprecate from GVoice 1.1.14
         * Callback when someone saied or silence in the same room.
         * to use nice for LOL, need callback with roomname
		 *
         * @param count: count of members who's status has changed.
         * @param members: a int array composed of [memberid_0, status,memberid_1, status ... memberid_2*count, status]
         * here, status could be 0, 1, 2. 0 meets silence and 1/2 means saying
         */
        virtual void OnMemberVoice(const char* roomName, const unsigned int *members, int count) ;
        ...
        /**
         * Callback when device event
		 *
		 * @param code : GV_ON_DEVICE_EVENT_ADD/GV_ON_DEVICE_EVENT_UNUSABLE/GV_ON_DEVICE_EVENT_DEFAULTCHANGE
		 * @param devType : 0:render, 1:capture
		 * @param device : device uuid
		 * @return : void
         */
		virtual void OnDeviceEvent(GCloudVoiceCompleteCode code, int devType, const char* device);
